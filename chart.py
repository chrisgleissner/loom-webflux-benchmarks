...
class LatencyMetrics:
    ...
    def _calculate_percentiles_and_errors(self):
        ...
        for time, status_code, latency in zip(self.seconds_elapsed, self.status_codes, self.latencies):
            if int(status_code / 100) != 2:
                self.error_seconds_elapsed.append(time)
    ...
    def _calculate_rps(self):
        ...
        if len(self.error_latencies) > 0:
            self.rps_error, _ = np.histogram(self.error_seconds_elapsed, bins=self.rps_bin_count,
                                             range=(0, self.rps_bin_count))
...
class SystemMetrics:
    ...
    def __init__(self, filename):
        ...
        self.total_cpu = [user + system + iowait for user, system, iowait in
                          zip(self.user_cpu, self.system_cpu, self.iowait_cpu)]
...
class ScenarioChart:
    ...
    def _add_request_plots(self, latency_metrics, latency_plot, rps_plot):
        latency_plot.scatter(latency_metrics.seconds_elapsed, latency_metrics.latencies,
                             label=legend_label('Latency', latency_metrics.latencies, 'ms'), color='silver',
                             alpha=0.7, s=1)
        latency_plot.scatter(latency_metrics.error_seconds_elapsed, latency_metrics.error_latencies,
                             label='Errors: ' + str(len(latency_metrics.error_latencies)),
                             color='red')

        latency_plot.plot(latency_metrics.percentile_time_buckets, latency_metrics.p99_values, label='p99: {:,.0f}ms'
                          .format(np.percentile(latency_metrics.latencies, 99)), color='blue', linewidth=1)
        latency_plot.plot(latency_metrics.percentile_time_buckets, latency_metrics.p90_values, label='p90: {:,.0f}ms'
                          .format(np.percentile(latency_metrics.latencies, 90)), color='green', linewidth=1)
        latency_plot.plot(latency_metrics.percentile_time_buckets, latency_metrics.p50_values, label='p50: {:,.0f}ms'
                          .format(np.percentile(latency_metrics.latencies, 50)), color='black', linewidth=2)
        ...
        rps_plot.plot(list(range(latency_metrics.rps_bin_count)), latency_metrics.rps,
                      label=legend_label('RPS', latency_metrics.rps), color='black', linewidth=2)
        rps_plot.set_ylabel('Requests per second', color='black')

        title_postfix = '. Total requests: {:,.0f} ok, {:,.0f} failed.'.format(
            len(latency_metrics.latencies) - len(latency_metrics.error_latencies),
            len(latency_metrics.error_latencies))
        rps_plot.set_title(
            title_label(prefix='RPS', measurements=latency_metrics.rps, unit='', postfix=title_postfix), weight='bold')
    ...
    def _add_system_plots(self, system_metrics, jvm_metrics, cpu, ram, socket, throughput):
        ...
        title_postfix = ('. JVM platform threads: {:,.0f} avg, {:,.0f} max. {:,.0f} GCs totalling {:,.1f}s'
                         .format(np.average(jvm_metrics.platform_thread_count),
                                 np.max(jvm_metrics.platform_thread_count),
                                 jvm_metrics.gc_counts[-1], jvm_metrics.gc_times[-1] / 1000))
        cpu.set_title(title_label(prefix='CPU', measurements=system_metrics.total_cpu, unit='%', postfix=title_postfix),
                      weight='bold')

        ram.plot(jvm_metrics.seconds_elapsed, jvm_metrics.heap_used,
                 label=legend_label('JVM Heap Used', jvm_metrics.heap_used, '%'),
                 color='black', linestyle='solid', linewidth=1)
        ram.plot(system_metrics.seconds_elapsed, system_metrics.mem_used,
                 label=legend_label('RAM Used', system_metrics.mem_used, '%'),
        ...
        socket.plot(system_metrics.seconds_elapsed, system_metrics.tcpsck,
                    label=legend_label('TCP', system_metrics.tcpsck),
                    linestyle='solid', linewidth=2)
        socket.plot(system_metrics.seconds_elapsed, system_metrics.active_s,
                    label=legend_label('TCP Active Opens / s', system_metrics.active_s),
                    linestyle='solid', linewidth=1)
        socket.plot(system_metrics.seconds_elapsed, system_metrics.passive_s,
                    label=legend_label('TCP Passive Opens / s', system_metrics.passive_s),
                    linestyle='solid', linewidth=1)
        ...
        throughput.plot(system_metrics.seconds_elapsed, system_metrics.total_seg_s,
                        label=legend_label('Total', system_metrics.total_seg_s, ''),
                        color='black', linestyle='solid', linewidth=2)
        throughput.plot(system_metrics.seconds_elapsed, system_metrics.iseg_s,
                        label=legend_label('Received / s', system_metrics.iseg_s),
                        color='tab:gray', linestyle='solid', linewidth=1)
        throughput.plot(system_metrics.seconds_elapsed, system_metrics.oseg_s,
                        label=legend_label('Sent / s', system_metrics.oseg_s),
                        color='tab:green', linestyle='solid', linewidth=1)
...
def title_label(prefix, measurements, unit={}, postfix=''):
    return ('{} (Min / Avg / Max: {:,.0f} / {:,.0f} / {:,.0f}{}{})'
            .format(prefix, np.min(measurements), np.average(measurements), np.max(measurements), unit, postfix))


def legend_label(name, measurements, unit=''):
    return ('{}: {:,.0f} / {:,.0f} / {:,.0f}{}'
            .format(name, np.min(measurements), np.average(measurements), np.max(measurements), unit))
...
def main():
    if len(sys.argv) != 6:
        ...
        print(datetime.now().strftime("%H:%M:%S") + " Created scenario chart in",
              (int)((time.time() - start_time) * 1000), "ms")
